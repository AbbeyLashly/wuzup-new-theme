...
class Broden_Latest_Posts_Widget extends WP_Widget {

	public function Broden_Latest_Posts_Widget() {
		$widget_ops = array(
			'classname' => 'latest-posts-widget',
			'description' => esc_html__( 'Display Latest Posts', 'broden' )
		);

		parent::__construct(
			'broden_latest_posts_widget',
			esc_html__( 'Broden - Latest Posts', 'broden' ),
			$widget_ops
		);
	}
	...
	public function widget( $args, $instance ) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'broden_latest_posts_widget', 'widget' );
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'broden_latest_posts_widget', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( isset( $alloptions[ 'broden_latest_posts_widget' ] ) ) {
			delete_option( 'broden_latest_posts_widget' );
		}
...
add_action( 'widgets_init', 'broden_latest_posts_widget' );

function broden_latest_posts_widget() {
	register_widget( 'Broden_Latest_Posts_Widget' );
}